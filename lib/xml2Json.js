      ...
      parser.parseString(xmlString, (err, data) => {
        ...
        const arrayEmitter = (childArray) => {
          for (let i = 0; i < childArray.length; i += 1) {
            if (Array.isArray(childArray[i])) {
              arrayEmitter(childArray[i]);
            } else {
              self.emit('data', messages.newMessage(childArray[i]));
            }
          }
        };
        ...
        const objectEmitter = (childObj) => {
          ...
          for (const child of childElems) {
            if (Array.isArray(childObj[child])) {
              arrayEmitter(childObj[child]);
            } else if (typeof childObj[child] === 'object') {
              objectEmitter(childObj[child]);
            } else {
              self.emit('data', messages.newMessage(childObj[child]));
            }
          }
        ...
        const transformedResults = cfg.customJsonata ? transform(data, { customMapping: cfg.customJsonata }, false) : transform(data, cfg, true);

        if (cfg.splitResult) {
          objectEmitter(transformedResults);
        } else {
          self.logger.info('Successfully converted XML to JSON');
