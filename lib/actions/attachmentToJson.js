...
const { getUserAgent, MAX_FILE_SIZE, memUsage } = require('../utils');
...
module.exports.process = async function processAction(msg, cfg) {
  ...
  for (const file of files) {
    ...
    const response = await attachmentProcessor.getAttachment(file.url, 'text');
    this.logger.debug(`For provided filename response status: ${response.status}`);
    const responseBodyString = response.data;
    ...
    await self.emit('data', returnMsg);
  }
};
