...
function checkFileName(self, fileName, pattern) {
  ...
  if (fileName.split('.').pop() !== 'xml') {
    self.logger.debug('Provided is not .xml file: ', fileName);
    return false;
  }
...
module.exports.process = async function processAction(msg, cfg) {
  ...
  for (const key of Object.keys(attachments)) {
    ...
    if (checkFileName(self, fileName, pattern)) {
      if (fileSize === undefined || fileSize < MAX_FILE_SIZE) {
        ...
        this.logger.debug(`For filename ${fileName} Response status: ${response.status}`);
        ...
        if (fileSize < MAX_FILE_SIZE) {
          ...
          this.logger.debug(`For filename ${fileName} Result of XML Conversed`);
          foundXML = true;
  ...
  if (!foundXML) {
    self.logger.info('No XML files that match the pattern found with in attachments');
  }
};
