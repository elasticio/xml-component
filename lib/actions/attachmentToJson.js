...
module.exports.process = async function processAction(msg, cfg) {
  ...
  for (const file of files) {
    ...
    const response = await attachmentProcessor.getAttachment(file.url, 'arraybuffer');
    ...
    const fileSize = sizeof(responseBodyString);
    if (fileSize > MAX_FILE_SIZE) throw new Error(tooLargeErrMsg(file.fileName || '', fileSize));
